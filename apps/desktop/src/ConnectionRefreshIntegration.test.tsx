...
test(
  "surfaces a GitHub post-authorization refresh failure through App",
  async () => {
    let githubReads = 0;
...
test(
  "surfaces a GitLab PAT refresh failure through App after connection succeeds",
  async () => {
    let gitlabReads = 0;
    mocks.invoke.mockImplementation((command: string) => {
      if (command === "dashboard") return Promise.resolve(emptyDashboard());
    ...
    fireEvent.click(
      await screen.findByRole("tab", { name: "Self-managed" }),
    );
    fireEvent.change(
    ...
    expect(
      await screen.findByText(/GitLab refresh failed/),
    ).toBeInTheDocument();
